...
local PreCombat = {
	...
	{'Summon Pet', '!talent(6,1)'},
 	--actions.precombat+=/summon_infernal,if=talent.grimoire_of_supremacy.enabled&artifact.lord_of_flames.rank>0
	{'Summon Infernal', 'talent(6,1)&artifact(Lord of Flames).rank>0'},
 	--actions.precombat+=/summon_infernal,if=talent.grimoire_of_supremacy.enabled&active_enemies>=3
	{'Summon Infernal', 'talent(6,1)&player.area(30).enemies>=3'},
	...
	{'Summon Doomguard', 'talent(6,1)&player.area(30).enemies<3&artifact(Lord of Flames).rank=0'},
 	--actions.precombat+=/augmentation,type=defiled
 	--actions.precombat+=/snapshot_stats
 	--actions.precombat+=/potion,name=deadly_grace
 	--actions.precombat+=/demonic_empowerment
	{'Demonic Empowerment', '!prev(Demonic Empowerment)'},
...
local Util = {
	...
	{'Implosion', 'talent(2,3)&{warlock(Wild Imp).remaining_duration<=action(Demonbolt).execute_time&player.buff(Demonic Synergy).remains}'},
	...
	{'Implosion', 'talent(2,3)&{warlock(Wild Imp).count<=4&warlock(Wild Imp).remaining_duration<=action(Demonbolt).execute_time&target.area(8).enemies>1}'},
	...
	{'Shadowflame', 'talent(2,1)&{target.debuff(Shadowflame)react<action(Demonbolt).cast_time+travel_time(Shadow Flame)}'},
	...
	{'Hand of Gul\'dan', 'soul_shard>=4&!talent(7,1)'},
	...
	{'Summon Darkglare', 'talent(7,1)&{cooldown(Call Dreadstalkers).remains>5}'},
 	--actions+=/summon_darkglare,if=cooldown.call_dreadstalkers.remains<=action.summon_darkglare.cast_time&soul_shard>=3
	{'Summon Darkglare', 'talent(7,1)&{cooldown(Call Dreadstalkers).remains<=action(Summon Darkglare).cast_time&soulshard>=3}'},
	...
	{'Summon Darkglare', 'talent(7,1)&{cooldown(Call Dreadstalkers).remains<=action(Summon Darkglare).cast_time&soulshard>=1&player.buff(Demonic Calling).react}'},
	...
	{'Hand of Gul\'dan', 'soul_shard>=3&prev_gcd(Call Dreadstalkers)'},
 	--actions+=/hand_of_guldan,if=soul_shard>=5&cooldown.summon_darkglare.remains<=action.hand_of_guldan.cast_time
	{'Hand of Gul\'dan', 'talent(7,1)&{soul_shard>=6&cooldown(Summon Darkglare).remains<=action(Hand of Gul\'dan).cast_time}'},
 	--actions+=/hand_of_guldan,if=soul_shard>=4&cooldown.summon_darkglare.remains>2
	{'Hand of Gul\'dan', 'talent(7,1)&{soul_shard>=6&cooldown(Summon Darkglare).remains>2}'},
 	--actions+=/demonic_empowerment,if=wild_imp_no_de>3|prev_gcd.hand_of_guldan
	{'Demonic Empowerment', 'warlock(Wild Imp).no_de>3||prev_gcd(Hand of Gul\'dan)'},
 	--actions+=/demonic_empowerment,if=dreadstalker_no_de>0|darkglare_no_de>0|doomguard_no_de>0|infernal_no_de>0|service_no_de>0
	{'Demonic Empowerment', 'warlock(Dreadstalker).no_de>0||warlock(Darkglare).no_de>0||warlock(Doomguard).no_de>0||warlock(Infernal).no_de>0||warlock(Felguard).no_de>0'},
...
--/dump NeP.DSL:Get('spell.exists')('player', 'Shadowbolt')
