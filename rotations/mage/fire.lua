...
local Talents = {
	...
	{'Living Bomb', 'talent(6,1)&player.area(40).enemies>3&!player.buff(Combustion)'}
}
...
local MainRotation = {
	...
	{'Phoenix\'s Flames', 'artifact(Phoenix\'s Flames).equipped&{action(Phoenix\'s Flames).charges>2.7&player.area(40).enemies>2}'},
	--actions.single_target+=/flamestrike,if=talent.flame_patch.enabled&active_enemies>2&buff.hot_streak.react
	{'Flamestrike', 'talent(6,3)&player.area(40).enemies>2&player.buff(Hot Streak!)'},
  ...
  --{'!Fire Blast', 'talent(7,1)&player.buff(Heating Up)&player.casting(Fireball).left<98&!prev_off_gcd(Fire Blast)&{!talent(3,2)||action(Fire Blast).charges>1.5||{cooldown(Combustion).remains<40}}&{3-action(Fire Blast).charges}*{18*{spell_haste}}<=cooldown(Combustion).remains||target.time_to_die.remains<4'},
	...
	{'Phoenix\'s Flames', '{player.buff(Combustion)||player.buff(Rune of Power)||player.buff(Incanter\'s Flow).stack>3||talent(3,1)}&{4-action(Phoenix\'s Flames).charges}*13<cooldown(Combustion).remains+5||target.time_to_die.remains<10'},
...
NeP.CR:Add(63, '[|cff'..Xeer.addonColor..'Xeer|r] MAGE - Fire', inCombat, outCombat, exeOnLoad)
